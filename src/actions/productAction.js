...
export const getProduct =
  (keyword = "", currentPage = 1, price = [0, 125500], category, ratings = 0) =>
  async (dispatch) => {
    try {
      dispatch({
...
export const createProduct = (productData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(`${server}/api/v1/admin/product/new`, productData, config);
...
export const updateProduct = (id,productData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(`${server}/api/v1/admin/product/${id}`, productData, config);
